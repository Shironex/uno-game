  ...
  Game.players.forEach((player, index) => {
    if (player.name === data.playerName && player.name === Game.currentPlayerTurn) {
      ...
      if (cardIndex !== -1) {    
        ...
        Game.discardPile.push({ src: data.card, ...(data.color && { color: data.color }) });
        ...
        if (data.effect) 
        {
          ...
          switch (data.effect) {
            case "D4":
              Game.players[nextPlayerIndex].deck = [...Game.players[nextPlayerIndex].deck, ...Game.drawPile.slice(-4)];
              Game.drawPile = Game.drawPile.slice(0, -4);
              break;
            case "D2":
              Game.players[nextPlayerIndex].deck = [...Game.players[nextPlayerIndex].deck, ...Game.drawPile.slice(-2)];
              Game.drawPile = Game.drawPile.slice(0, -2);
              break;
            case "Skip":
  ...
  Game.players.forEach((player, index) => {
    if (player.name === data.playerName && player.name === Game.currentPlayerTurn) {
       ...
       if (!helpers.isSpecialCard(NewCard.src) && helpers.isValidCard(NewCard.src, TopCard))
       {
          ...
          socket.emit("Card-Placed", Game.discardPile[Game.discardPile.length - 1].src);
